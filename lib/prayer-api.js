...
/**
 * Get daily prayer counts for a specific month
 * @param {number} year - Year (e.g., 2024)
 * @param {number} month - Month (1-12, where 1 = January, 12 = December)
 * @returns {Promise<Array>} Array of daily prayer counts with day, date, and prayers count
 */
export async function getDailyPrayerCounts(year, month) {
  try {
    const response = await fetch(
      `/api/prayers/daily-counts?year=${year}&month=${month}`,
      {
        headers: getAuthHeaders(),
      }
    );
    ...
    if (!response.ok) {
      throw new Error(data.error || "Failed to fetch daily prayer counts");
    }
